...
async function installExtension(options) {
    const { projectId, extensionName, source, extVersion, paramFilePath, nonInteractive, force, } = options;
    ...
    try {
        ...
        if (roles.length) {
            ...
            const consented = await extensionsHelper_1.confirmInstallInstance(nonInteractive, force);
            if (!consented) {
        ...
        switch (choice) {
            case "installNew":
                ...
                params = await paramHelper.getParams(projectId, _.get(spec, "params", []), nonInteractive, paramFilePath);
...
exports.default = new command_1.Command("ext:install [extensionName]")
    ...
    const confirm = await extensionsHelper_1.confirmInstallInstance(options.nonInteractive, options.force);
